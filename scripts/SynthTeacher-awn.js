/* Declares the SynthTeacher Audio Worklet Node */

class SynthTeacherController extends WAMController
{
  constructor (actx, options) {
    ...
    super(actx, "SynthTeacher", options);
  }
  ...
  static importScripts (actx) {
    ...
    return new Promise( (resolve) => {
      actx.audioWorklet.addModule(origin + "scripts/SynthTeacher-wam.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/wam-processor.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/SynthTeacher-awp.js").then(() => {
        resolve();
      }) }) });
    })
  }
